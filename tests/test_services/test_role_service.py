...
def role_service(db_session: AsyncSession, role_crud):
    ...
    return RoleService(db_session, role_crud)


@pytest.fixture
def service(db_session: AsyncSession):
    return RoleService(db_session, role_crud_instance)
...
class TestRoleServiceCreate:
    ...
    async def test_create_role_success(self, service: RoleService):
        """测试成功创建角色"""
        role_in = RoleCreate(name="Service Role", code="svc_role", description=None, sort=0, menu_ids=None)
    ...
    async def test_create_role_duplicate_code(self, service: RoleService):
        """测试重复编码报错"""
        role_in = RoleCreate(name="Role 1", code="uniq_code", description=None, sort=0, menu_ids=None)
        await service.create_role(obj_in=role_in)

        # 尝试使用相同 code 创建
        role_in_2 = RoleCreate(name="Role 2", code="uniq_code", description=None, sort=0, menu_ids=None)
...
class TestRoleServiceUpdate:
    ...
    async def test_update_role_success(self, service: RoleService):
        role_in = RoleCreate(name="To Update", code="to_update", description=None, sort=0, menu_ids=None)
        role = await service.create_role(obj_in=role_in)
    ...
    async def test_update_role_duplicate_code(self, service: RoleService):
        """更新时编码冲突检查"""
        # 创建两个角色
        await service.create_role(obj_in=RoleCreate(name="R1", code="c1", description=None, sort=0, menu_ids=None))
        r2 = await service.create_role(obj_in=RoleCreate(name="R2", code="c2", description=None, sort=0, menu_ids=None))
    ...
    async def test_update_role_same_code(self, service: RoleService):
        """更新自己时不应报冲突"""
        r1 = await service.create_role(obj_in=RoleCreate(name="R1", code="c1", description=None, sort=0, menu_ids=None))
