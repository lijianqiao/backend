...
_REVOKED_JTI: Final[str] = "__revoked__"
_REVOKED_AFTER_TTL_SECONDS: Final[int] = 30 * 24 * 3600  # 访问失效阈值的保存时长（30天），用于即时失效对比
...
class RefreshTokenStore:
    async def set_current_jti(self, user_id: UserId, jti: Jti, ttl_seconds: Seconds) -> None:
        raise NotImplementedError

    async def get_current_jti(self, user_id: UserId) -> Jti | None:
        raise NotImplementedError

    async def revoke_user(self, user_id: UserId) -> None:
        raise NotImplementedError

    async def revoke_users(self, user_ids: Iterable[UserId]) -> None:
        for uid in user_ids:
            await self.revoke_user(uid)


def _refresh_key(user_id: UserId) -> str:
...
def _revoked_after_key(user_id: UserId) -> str:
    return f"v1:auth:revoked_after:{user_id}"


class AccessGate:
    async def set_revoked_after(self, user_id: UserId, ts_seconds: Timestamp) -> None:  # pragma: no cover
        raise NotImplementedError

    async def get_revoked_after(self, user_id: UserId) -> Timestamp | None:  # pragma: no cover
        raise NotImplementedError

    async def revoke_now(self, user_id: UserId) -> None:  # pragma: no cover
        raise NotImplementedError

    async def revoke_many_now(self, user_ids: Iterable[UserId]) -> None:  # pragma: no cover
        for uid in user_ids:
            await self.revoke_now(uid)


class RedisAccessGate(AccessGate):
    async def set_revoked_after(self, user_id: UserId, ts_seconds: Timestamp) -> None:
        if cache.redis_client is None:
            return
    ...
    async def get_revoked_after(self, user_id: UserId) -> Timestamp | None:
        if cache.redis_client is None:
            return None
        key = _revoked_after_key(user_id)
        try:
    ...
    async def revoke_now(self, user_id: UserId) -> None:
...
class MemoryAccessGate(AccessGate):
    def __init__(self) -> None:
        ...
        self._data: dict[UserId, tuple[Timestamp, Timestamp]] = {}
...
class MemoryRefreshTokenStore(RefreshTokenStore):
    ...
    def __init__(self) -> None:
        ...
        self._data: dict[UserId, tuple[Jti, Timestamp]] = {}
...
async def set_user_refresh_jti(*, user_id: UserId, jti: Jti, ttl_seconds: Seconds) -> None:
    await get_refresh_token_store().set_current_jti(user_id, jti, ttl_seconds)


async def get_user_refresh_jti(*, user_id: UserId) -> Jti | None:
    return await get_refresh_token_store().get_current_jti(user_id)


async def revoke_user_refresh(*, user_id: UserId) -> None:
    await get_refresh_token_store().revoke_user(user_id)


async def revoke_users_refresh(*, user_ids: Iterable[UserId]) -> None:
...
async def set_user_revoked_after(*, user_id: UserId, ts_seconds: Timestamp) -> None:
    await get_access_gate().set_revoked_after(user_id, ts_seconds)


async def get_user_revoked_after(*, user_id: UserId) -> Timestamp | None:
    return await get_access_gate().get_revoked_after(user_id)


async def revoke_user_access_now(*, user_id: UserId) -> None:
    await get_access_gate().revoke_now(user_id)


async def revoke_users_access_now(*, user_ids: Iterable[UserId]) -> None:
